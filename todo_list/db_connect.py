...
class TodoUser(Base, SoftDeleteMixin):
    ...
    sent_requests = relationship(
        "Friends", foreign_keys="[Friends.requester_id]", back_populates="requester"
    )
    received_requests = relationship(
        "Friends", foreign_keys="[Friends.addressee_id]", back_populates="addressee"
    )
...
class Todo(Base):
    ...
    # 로그인한 사용자 연결용 외래키
    user_id = Column(
        Integer, ForeignKey("todo_user.id", ondelete="CASCADE"), nullable=False
    )
    # 관계 설정
...
class Friends(Base):
    ...
    requester = relationship(
        "TodoUser", foreign_keys=[requester_id], back_populates="sent_requests"
    )
    addressee = relationship(
        "TodoUser", foreign_keys=[addressee_id], back_populates="received_requests"
    )
