...
def add_like(
        ...
        .filter(
            (
                (
                    ...
                    & (Friends.addressee_id == todo.user_id)
                )
                | (
                    (Friends.requester_id == todo.user_id)
    ...
    existing_like = (
        db.query(Like)
        .filter(Like.user_id == current_user.id, Like.todo_id == todo_id)
        .first()
    )
    if existing_like:
