...
def send_friend_request(
    ...
    existing = (
        ...
        .filter(
            or_(
                ...
                & (Friends.addressee_id == addressee_id),
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail="Friend request already exists or already friends"
        )
...
def accept_friend_request(
    ...
    request = (
        db.query(Friends)
        .filter(Friends.id == request_id, Friends.addressee_id == current_user.id)
        .first()
    )
    if not request:
...
def get_friend_requests(
    ...
    requests = (
        db.query(Friends)
        .filter(Friends.addressee_id == current_user.id, Friends.status == "pending")
        .all()
    )
...
def delete_friend(
    ...
    friendship = (
        ...
        .filter(
            or_(
                (Friends.requester_id == current_user.id)
                & (Friends.addressee_id == friend_id),
                (Friends.requester_id == friend_id)
                & (Friends.addressee_id == current_user.id),
            ),
            Friends.status == "accepted",
        )
    ...
    db.query(Like).filter(
        Like.user_id == current_user.id, Like.todo_id.in_(friend_todo_ids)
    ...
    db.query(Like).filter(
        Like.user_id == friend_id, Like.todo_id.in_(current_user_todo_ids)
    ).delete(synchronize_session=False)
...
def get_friend_public_todos(
    ...
    todos = (
        db.query(Todo).filter(Todo.user_id == friend_id, Todo.publicity == True).all()
    )
...
def search_user_status(
    ...
    if user.id == current_user.id:
        return {"status": "self", "user": {"id": user.id, "name": user.name}}
    ...
    if not relation:
        return {"status": "none", "user": {"id": user.id, "name": user.name}}

    return {"status": relation.status, "user": {"id": user.id, "name": user.name}}
...
def get_all_friends_public_todos(
    ...
    friend_ids = [
        f.requester_id if f.requester_id != current_user.id else f.addressee_id
        for f in friends
    ]

    friend_users = db.query(TodoUser).filter(TodoUser.id.in_(friend_ids)).all()
    friend_user_map = {user.id: user for user in friend_users}

    todos = (
        db.query(Todo)
        .filter(Todo.user_id.in_(friend_ids), Todo.publicity == True)
        .all()
    )
    ...
    for todo in todos:
        print("--------------------------")
        if todo.user_id not in todos_by_friend:
            todos_by_friend[todo.user_id] = []
        todos_by_friend[todo.user_id].append(
            {
                "todo_id": todo.id,
                "title": todo.title,
    ...
    for friend_id in friend_ids:
        print("-==========================")
        friend_user = friend_user_map.get(friend_id)
        if friend_user:
            result.append(
                {
                    "friend_id": friend_id,
                    "friend_name": friend_user.name,
                    "todos": todos_by_friend.get(friend_id, []),
                }
            )
